...
def format_swc_response(fullcc: str, result: dict, site: str, user_info: dict, time_taken: float) -> str:
    """Format the SWC response with professional billing info."""
    parts = fullcc.split("|")
    ...
    # Site display
    site_display = site.split('/')[2] if site and '/' in site else site or "epicalarc.com"
    
    return f"""<b>╔══════════════════════════╗
║     𝐒𝐓𝐑𝐈𝐏𝐄 𝐖𝐂 𝐀𝐔𝐓𝐇 {status_emoji}     
╚══════════════════════════╝</b>
...
<b>┌─────── GATEWAY INFO ───────┐</b>
│ <b>Gateway:</b> <code>Stripe WooCommerce</code>
│ <b>Site:</b> <code>{site_display}</code>
...
━━━━━━━━━━━━━━━
<b>Usage:</b>
<code>/swc cc|mm|yy|cvv</code>
<code>/swc cc|mm|yy|cvv https://site.com</code>

<b>Examples:</b>
<code>/swc 4111111111111111|12|25|123</code>
<code>/swc 4111111111111111|12|25|123 https://epicalarc.com</code>""",
                reply_to_message_id=message.id,
                parse_mode=ParseMode.HTML
            )
...
<b>Example:</b> <code>/swc 4111111111111111|12|25|123</code>""",
                reply_to_message_id=message.id,
                parse_mode=ParseMode.HTML
            )
...
<b>Status:</b> <i>Authenticating...</i>""",
        ...
        # Format response
        final_message = format_swc_response(fullcc, result, site, user_info, time_taken)
...
# Callback handler for approved info
@Client.on_callback_query(filters.regex("^approved_info$"))
async def approved_info_callback(client, callback_query):
    """Show info about approved card."""
    await callback_query.answer(
        "✅ Card authentication successful! Card is valid.",
        show_alert=True
    )
