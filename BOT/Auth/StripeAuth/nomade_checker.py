        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                ...
                async with session.get(
                    f'{self.site_url}/my-account/',
                    headers=self._get_headers(f'{self.site_url}/my-account/')
                ) as resp:
                    await resp.text()
                    self.session_cookies.update(resp.cookies)
                ...
                async with session.get(
                    ...
                    headers=self._get_headers(f'{self.site_url}/my-account/payment-methods/')
                ) as resp:
                ...
                if "error" in pm_result:
                    ...
                    if error_code in ccn_live_codes or any(x in error_msg.lower() for x in ["cvc", "security code", "zip", "postal", "insufficient"]):
                        result["success"] = True
                ...
                try:
                    ...
                    if confirm_json.get("success") == True:
                        ...
                        ccn_patterns = [
                            ...
                            "AUTHENTICATION", "3D SECURE", "3DS", "AUTHENTICATION_REQUIRED",
                    ...
                    if "success" in confirm_lower or "thank you" in confirm_lower or "approved" in confirm_lower:
                        result["success"] = True
                        result["response"] = "APPROVED"
